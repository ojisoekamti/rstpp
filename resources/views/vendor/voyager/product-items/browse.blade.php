                                    ...
                                    @foreach ($dataTypeContent as $data)
                                        <tr>
                                            ...
                                            @foreach ($dataType->browseRows as $row)
                                                ...
                                                <td id="{{ $row->field }}-{{ $data->id }}">
                                                    ...
                                                    @elseif($row->type == 'media_picker')
                                                        ...
                                                        @else
                                                            {{ trans_choice('voyager::media.files', 0) }}
                                                        @endif
                                                    @elseif($row->type == 'number' && $row->field == 'stock')
                                                        {{ $data->{$row->field} }}
                                                    @else
                                                        @include('voyager::multilingual.input-hidden-bread-browse')
                                                ...
                                                @if ($row->type == 'number' && $row->field == 'stock')
                                                    <td>
                                                        <button class="btn btn-sm btn-primary minus"
                                                            onclick="updateStock('{{ $data->id }}', -1)">-</button>
                                                        <button class="btn btn-sm btn-danger plus"
                                                            onclick="updateStock('{{ $data->id }}', 1)">+</button>
...
@section('javascript')
    ...
    <script>
        function updateStock(productId, delta) {
            fetch(`/api/product-items/${productId}/update-stock`, {
                    method: 'PATCH',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        stockElement.textContent = parseInt(stockElement.textContent) + delta;
                    } else {
                        alert(data.message);
                    }
                })
